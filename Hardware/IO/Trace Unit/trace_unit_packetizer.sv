        ...
        always_comb begin
            ...
            if (trace_interface_i.valid & enable_event_tracing_i) begin
                case (trace_interface_i.info)
                    `INSTR_MISALIGNED: event_generate = enable_i.instr_misaligned;

                    `INSTR_ACCESS_FAULT: event_generate = enable_i.instr_access_fault;

                    `INSTR_ILLEGAL: event_generate = enable_i.instr_illegal;

                    `BREAKPOINT: event_generate = enable_i.breakpoint;

                    `LOAD_OPERATION: event_generate = enable_i.load_operation;

                    `LOAD_MISALIGNED: event_generate = enable_i.load_misaligned;

                    `LOAD_ACCESS_FAULT: event_generate = enable_i.load_access_fault;

                    `STORE_OPERATION: event_generate = enable_i.store_operation;

                    `STORE_MISALIGNED: event_generate = enable_i.store_misaligned;

                    `STORE_ACCESS_FAULT: event_generate = enable_i.store_access_fault; 

                    `U_SYSTEM_CALL: event_generate = enable_i.u_system_call;

                    `M_SYSTEM_CALL: event_generate = enable_i.m_system_call;

                    `SLEEP: event_generate = enable_i.sleep;

                    `HANDLER_RETURN: event_generate = enable_i.handler_return;

                    `BRANCH_OPERATION: event_generate = enable_i.branch;

                    `JUMP_OPERATION: event_generate = enable_i.jump;

                    `CSR_OPERATION: event_generate = enable_i.csr;

                    `NO_EVENT: event_generate = 1'b0;

                    default: event_generate = 1'b0;
                endcase
            end
        end
...
//====================================================================================
    ...
    logic write_packet; trace_unit_packet_t trace_packet;
        ...
        always_comb begin
            ...
            /* Priority: Overflow > Divergence > Event */
            ...
            if (overflow) begin
                ...
                write_packet = !halt_core_o;
            end else if (event_generate) begin
