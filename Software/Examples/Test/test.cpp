 ...
 */
    ...
    SPI spi(0); SPI::spiError_e error = SPI::NO_ERROR;
    ...
    uint64_t data64 = spi.exchange((uint64_t) 0xF0F0F0F0F0F0F0F0);

    /* Send 32 bit */
    print("Sending 32 bits...\n", uart);
    uint32_t data32 = spi.exchange((uint32_t) 0xF0F0F0F0);

    /* Send 16 bit */
    print("Sending 16 bits...\n", uart);
    uint16_t data16 = spi.exchange((uint16_t) 0xF0F0);

    /* Send 8 bit */
    print("Sending 8 bits...\n", uart);
    uint8_t data8 = spi.exchange((uint8_t) 0xF0);

    /* Send array of bytes */
    uint8_t byteArrayTX[] = "SPI Transfer...\n";
    uint8_t byteArrayRX[17];
    spi.exchangeStream(byteArrayTX, byteArrayRX, 17);
